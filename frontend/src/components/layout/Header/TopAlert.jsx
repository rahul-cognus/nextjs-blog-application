...
const TopAlert = () => {
  const [hideStrip, setHideStrip] = useState(true);
  return (
    <>
      {hideStrip && (
        <div className="bg-primary py-2 text-center overflow-hidden relative">
          {/* SVG */}
          ...
          <div className=" relative">
            <div className="flex flex-col md:flex-row  justify-center items-center text-white text-15">
              ...
              <Link
                href={"/"}
                className="text-10 font-medium bg-gray-800 hover:bg-gray-700 transition-all ease-in-out text-white ms-3 mb-0 rounded-[4px] py-[7px] px-3"
              >
                Grab now!
              </Link>
            </div>
          ...
          <button
            onClick={() => setHideStrip(false)}
            className="absolute top-0 bottom-0 right-3"
          >
            <IoCloseOutline className="cursor-pointer text-white w-6 h-6" />
          </button>
        </div>
      )}
    </>
